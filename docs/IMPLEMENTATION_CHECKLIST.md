...
|-------|--------|------|--------|---------|------|
| **P0** | 前端调用cost-ranking API | 低 | 低 | 2h | 待实施 |
| **P1** | 补全异常分布查询 | 低 | 低 | 1h | 待实施 |
| **P1** | 补全Turn效率查询 | 低 | 低 | 2h | 待实施 |
| **P2** | 补全Subagent查询 | 中 | 中 | 3h | 待实施 |
| **P2** | 清理TTFT硬编码数据 | 低 | 低 | 1h | 待实施 |
| **P2** | 清理Turn硬编码卡片 | 低 | 低 | 1h | 待实施 |
...
### 第一阶段完成检查
- [ ] 改进项#1：前端调用cost-ranking API
- [ ] 改进项#2：异常分布真实查询
- [ ] 改进项#3：Turn效率真实查询
- [ ] 测试验证：成本归因Top10显示真实数据
- [ ] 测试验证：异常分布图更新
- [ ] 测试验证：Turn效率显示真实指标

### 第二阶段完成检查
- [ ] 改进项#4：Subagent成本真实查询
- [ ] 改进项#5：清理TTFT硬编码HTML
- [ ] 改进项#6：清理Turn硬编码卡片
- [ ] 改进项#7：清理Subagent硬编码统计
